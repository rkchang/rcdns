...
AsyncServer::AsyncServer(asio::io_context &io, int port, std::string &dns_address,
...
void AsyncServer::lookup(const uint16_t header_id, const std::string &qname,
...
void AsyncServer::handle_authority(const DnsPacket &recvd) {
...
void AsyncServer::handle_answer(const DnsPacket &recvd) {
